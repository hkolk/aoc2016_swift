...
let room = "ojvtpuvg"
var passphrase = [4: "C", 5:"B", 3:"0", 1:"0"]
for i in 10320580... {
    let result = MD5("\(room)\(i)")
    //print(result)
    if(result.hasPrefix("00000")) {
        print("Match! i = \(i), hash = \(result)")
        if let position = Int(result[5]) {
            if(position < 8 && !passphrase.keys.contains(position)) {
                passphrase[position] = result[6]
                let tempPass = passphrase.values.joined()
                print("passphrase till now: \(tempPass), \(passphrase)")
            }
        }
    }
    if(i % 1000000 == 0) {
