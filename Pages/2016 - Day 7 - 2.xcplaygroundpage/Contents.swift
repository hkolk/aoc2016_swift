...
func parseIntoParts(_ ip:String) -> (regular:[String], hyper:[String]) {
    var regular = Array<String>()
    var hyper = Array<String>()
    
    let parts = ip.components(separatedBy: CharacterSet(charactersIn: "[]"))
    for i in 0...parts.count - 1 {
        if(i % 2 == 0) {
            regular.append(parts[i])
        } else {
            hyper.append(parts[i])
        }
    }
    return (regular, hyper)
}

func findAba(_ parts:[String], invert:Bool = false) -> Array<String> {
    var accu = Array<String>()
    for part in parts {
        for index in 0...part.count-1 {
            if(part[index] == part[index+2] && part[index] != part[index+1]) {
                let word = (invert) ? "\(part[index+1])\(part[index])\(part[index+1])" : part[index..<index+3]
                accu.append(word)
            }
        }
    }
...
let ips = input2.split(separator: "\n").map { String($0) }

var counter = 0
for ip in ips {
    let (regular, hyper) = parseIntoParts(ip)
    let regularSet = Set(findAba(regular))
    let hyperSet = Set(findAba(hyper, invert:true))
    
    if(regularSet.intersection(hyperSet).count > 0) {
        //print("Valid aba: \(ip)")
        counter+=1
    }
