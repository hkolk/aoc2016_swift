...
func decompress(_ compressed:String) -> String {
    ...
    while(index < compressed.count) {
        var char = compressed[index]
        if(char == "(") {
            var first = ""
            while(compressed[index+1] != "x") {
                index += 1
            ...
            index += 1 // skip the x
            var second = ""
            while(compressed[index+1] != ")") {
                index += 1
                char = compressed[index]
                second.append(char)
            }
            index += 2 // skip the closure
            // Index is at the real text now
            let selector = Int(first)!
            let repeater = Int(second)!
            for _ in 1...repeater {
                accu.append(compressed[index..<index+selector])
            }
            index += selector
        } else {
            accu.append(char)
...
print(decompressV2("X(8x2)(3x3)ABCY"))
