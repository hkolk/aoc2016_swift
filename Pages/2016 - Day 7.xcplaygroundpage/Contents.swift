...
func containsAbba(_ parts:[String]) -> Bool {
    for part in parts {
        for index in 0...part.count-1 {
            if(part[index] == part[index+3] && part[index+1] == part[index+2] && part[index] != part[index+1]) {
                return true
            }
        }
    }
...
for ip in ips {
    ...
    if(containsAbba(regular) && !containsAbba(hyper)) {
        //print("Valid abba: \(ip)")
        counter+=1
    }
}
