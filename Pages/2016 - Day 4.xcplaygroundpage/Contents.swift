...
func decryptName(_ name:String, _ sectorId:Int) -> String {
    var parts = Array(name).map { Character(String($0))}
    for _ in 1...sectorId {
        parts = parts.map { char in
            switch char {
            case "z": return "a"
            case "-": return " "
            case " ": return " "
            default: return Character(Unicode.Scalar(char.asciiValue! + 1)!)
            }
        }
    }
    return parts.map { String($0)}.joined()
}
//let test = decryptName("qzmt-zixmtkozy-ivhz", 343)
//print(test)

var sectorSum = 0
for line in input2.split(separator: "\n") {
    let dashParts = line.split(separator: "-").map { String($0)}
    let finalParts = dashParts.last!.split(separator: "[").map { String($0) }
    let checksum = finalParts.last!.replacingOccurrences(of: "]", with: "")
    let sectorId = Int(finalParts.first!)!
    let words = dashParts[0...(dashParts.count - 2)].joined()
    //print("\(words), \(sectorId), \(checksum)")
    let sorted = Array(words).countForElements().sorted {lhs, rhs in
    ...
    //print(sorted)
    let generatedCheckSum = sorted[0...4].map { String($0.key) }.joined()
    //print(generatedCheckSum)
    if(generatedCheckSum == checksum) {
        //print("Match!")
        sectorSum += sectorId
        print(decryptName(dashParts[0...(dashParts.count - 2)].joined(separator: "-"), sectorId))
    }
}
print("SectorSum: \(sectorSum)")
